...
BEGIN_MESSAGE_MAP(PlaySound1, CWinThread)
	ON_THREAD_MESSAGE_MY(WM_PLAYSOUND_STARTPLAYING, OnStartPlaying)
	ON_THREAD_MESSAGE_MY(WM_PLAYSOUND_STOPPLAYING, OnStopPlaying)
	ON_THREAD_MESSAGE_MY(WM_PLAYSOUND_PLAYBLOCK, OnWriteSoundData)
	ON_THREAD_MESSAGE_MY(MM_WOM_DONE, OnEndPlaySoundData)
	ON_THREAD_MESSAGE_MY(WM_PLAYSOUND_ENDTHREAD,OnEndThread)
END_MESSAGE_MAP()
...
LRESULT PlaySound1::OnWriteSoundData(WPARAM wParam, LPARAM lParam)
{
	...
	if(((CVideoNetDlg*)dlg)->isAudioReceive==FALSE)
	{
